...
V9 只有在同一 Runner 上对已验收的 V8 基线与候选版本完成对照后，才接受效率提升结论。确定性审计运行 `30232461778` 记录：

| 测量路径 | V8 基线 | V9 候选 | 结果 |
|---|---:|---:|---:|
| `verify_all --profile all` 中位墙钟时间 | 12.065 s | 11.109 s | 1.09 倍 |
| `verify_all` 墙钟时间 p90 | 12.075 s | 11.124 s | 遥测 |
| 工作流路由 | 基线 | 候选 | 258.77 倍 |
| 5 MiB 求解器输出解析 | 基线 | 候选 | 1.02 倍 |
| 峰值 RSS 比值 | 1.00 倍 | 1.00 倍 | 上限 1.10 倍 |
...
已于 `2026-07-27T02:36:01.634707+00:00` 由确定性终验运行 `30232461778` 完成验证。
...
| Windows core | Python 3.10 与 3.13；最终结果记录于 Issue #29 |
| 受控变异探针 | 64/64 被识别 |
| 科学参考基准 | 8/8 通过 |
| 仓库 / 依赖安全发现 | 0 / 0 |
...
V9 最终提交只有在 [Issue #29](../../issues/29) 记录 Ubuntu/Windows/macOS × Python 3.10/3.13 正式 CI 成功后才被接受。机器可读证据：[`reports/CURRENT_MAIN_VERIFICATION.json`](reports/CURRENT_MAIN_VERIFICATION.json)。
