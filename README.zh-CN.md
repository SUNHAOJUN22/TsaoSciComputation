...
已于 `2026-07-26T09:06:04.404265+00:00` 由确定性终验运行 `30195815447` 完成验证。
...
| Windows core | Python 3.10 与 3.13；最终结果记录于 Issue #25 |
| 受控变异探针 | 64/64 被识别 |
| 科学参考基准 | 8/8 通过 |
| 仓库 / 依赖安全发现 | 0 / 0 |
...
V5 最终提交只有在 [Issue #25](../../issues/25) 记录 Ubuntu/Windows/macOS × Python 3.10/3.13 正式 CI 成功后才被接受。机器可读证据：[`reports/CURRENT_MAIN_VERIFICATION.json`](reports/CURRENT_MAIN_VERIFICATION.json)。
