...
def verify_core() -> int:
    returncode = run("tests and coverage", (PYTHON, "scripts/run_tests.py", "--coverage"))
    if returncode:
        return returncode
...
def _write_timing_report(path: Path, profile: str, elapsed: float) -> None:
    payload = {
        "schema_version": "1.0",
        "profile": profile,
        ...
        "claim_boundary": (
            "Local verification orchestration timing only; external scientific solver performance "
            "is not measured."
        ),
    }
...
def main() -> int:
    ...
    if args.timing_json is not None:
        _write_timing_report(args.timing_json, args.profile, elapsed)
    if returncode:
        return returncode
